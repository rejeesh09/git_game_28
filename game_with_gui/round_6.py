...
# Round_5() class is where the fifth round of the game takes place(i.e. round5_play() method in Round_5())
# so the round5_play() method has to be called from Round_6 and round6_play() from Round_7 and so on
########################################################################
import time
import sys
...
class Round_6(Round_5):
    def __init__(self,hold,custom_deal,tkntr_rt):
        ...
        super().round5_play()
        # similarly round6_play(to be defined){as well as super().__init__()}
        # can be called in __init__() of class Round_7(Round_6) and so on.
    ...
    def round6_lead_logic(self):
        # logic for opening turn of round
        
        self.turn_index=self.round6_lead_index
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round6 gui window for taking lead card input(i.e. if self.player leading the round)
            self.player_input=self.gui_handle.gui_round6_card_entry()
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            #15.######### var15
            self.round6_lead_card=self.inp_parse_check(self.player_input)
        else:
...
#         time.sleep(0.5)
        sys.stdout.write(self.round6_lead_card.show())
        ###############################################################  
        
        
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round6 lead card
        self.gui_handle.gui_card_played6(self.turn_index,self.round6_lead_card)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

        # adding inp to lst and dictionaries
        self.obj_played_card_lst.append(self.round6_lead_card)
        self.obj_dictn_of_highest_card_and_turn['suit']=[self.turn_index,self.round6_lead_card]

        self.obj_dictn_of_played_card_and_player[self.players_lst[self.turn_index]]\
        .append(self.round6_lead_card)

        self.obj_dictn_of_played_card_and_suit[self.round6_lead_card.suit()].append(self.round6_lead_card)
        
        # updating an additional dictionary from 18/09/2022 - not used
        self.obj_dictn_of_player_index_and_hand[self.turn_index].remove(self.round6_lead_card)
        
        #16.######### var16
        self.round6_lead_card_suit=self.round6_lead_card.suit()
        self.round6_lead_suit_index=self.suit_dictn[self.round6_lead_card_suit]
        #17.######### var17
        self.round6_highest_point_sofar=self.round6_lead_card.point()
        
        # removing card played from lst and dictn
        self.obj_deal_lst_copy[self.turn_index].remove(self.round6_lead_card)
        self.obj_dictn_of_cards_grouped[self.turn_index]\
                                        [self.round6_lead_suit_index].remove(self.round6_lead_card)    
    ###################################################################
    #round6_lead_logic() method end ###################################
    
    ###################################################################    
    def round6_follow_logic(self,suit_played):
        ...
        ############################################################
        #round6_follow_logic()'s point_sofar() function end ########


# round6_follow_logic() main body###################################
        ...
        if self.turn_index:
            ...
            if len(self.obj_dictn_of_cards_grouped[self.turn_index][self.x]) != 0:
                ...
                if abs(self.round6_lead_index - self.turn_index) == 2:
                    ...
                    if self.obj_dictn_of_cards_grouped[self.turn_index][self.x][-1].point()>\
                        self.round6_highest_point_sofar:
                        # updating highest point
                        self.round6_highest_point_sofar=self.card_played.point()
...
#                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
        ...
        # i.e self.turn_index==0
            ...
            played=False
#             time.sleep(0.5)
            if not len(self.obj_dictn_of_cards_grouped[0][self.round6_lead_suit_index]):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                    ...
                    self.check_val=self.gui_handle.gui_round6_trump_call_instance(self.turn_index)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                        ...
                        # the gui disp method of Widget() class in widget_manager module is called by its object
                        # gui_handle which was created earlier in __init__() of Deck()
                        self.gui_handle.gui_round6_trump_reveal(self.turn_index,\
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                # check conditions
                self.round6_highest_point_sofar=max(self.card_played.point(),self.round6_highest_point_sofar)
#??????????????????????????????????????????????????????????????????????????????????
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        self.gui_handle.gui_card_played6(self.turn_index,self.card_played)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    
    ###################################################################
    #round6_follow_logic() method end #################################
    
    def round6_play(self):
        # round6_play() method  #######################################
                
        while(len(self.obj_played_card_lst)<4):

            # calling the round6 lead_logic for the first turn
            if len(self.obj_played_card_lst)==0:                
                self.round6_lead_logic()
            ...
            self.round6_follow_logic(self.round6_lead_suit_index)


        # determining round7_lead_index
        ...
        self.round7_lead_index=key
        self.round7_lead_player=self.players_lst[key]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # the gui disp method of Widget() class in widget_manager module is called by its object
        # gui_handle which was created earlier in __init__() of Deck()
        self.gui_handle.gui_round6_summary(self.point_oppo_team,self.point_player_team,\
                                          self.round7_lead_player)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        
        time.sleep(0.5)
        print('\n')
        print(20*' '+'Mate:'+'{}'.format(self.obj_dictn_of_played_card_and_player['Mate'][5].show()))
        print('Oppo_left:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                       ['Oppo_left'][5].show()),end=' ')
        print(21*' '+'Oppo_right:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                               ['Oppo_right'][5].show()))
        print(20*' '+'{}:'.format(self.players_lst[0]),end=' ')
        print(self.obj_dictn_of_played_card_and_player[self.players_lst[0]][5].show())
        print("\nRound6 - starting from {}, counter_clockwise: ".format\
              (self.players_lst[self.round6_lead_index]),end=' ')        
        for i in self.obj_played_card_lst:
            print(i.show(),end=' ')
        print('')
        print('\nround7_lead_index: ',self.round7_lead_index)
