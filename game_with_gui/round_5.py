...
# Round_*4() class is where the fourth round of the game takes place(i.e. round*4_play() method in Round_*4())
# so the round*4_play() method has to be called from Round_*5 and round*5_play() from Round_*6 and so on
########################################################################
import time
import sys
...
class Round_5(Round_4):
    def __init__(self,hold,custom_deal,tkntr_rt):
        ...
        super().round4_play()
        # similarly round*5_play(to be defined){as well as super().__init__()} 
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round*5 gui window for taking lead card input(i.e. if self.player leading the round)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # point card or the minimum point card or a completely random card is played
            ...
            # debugging is going on
                ...
                # loading the previously played round*5 lead card
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round*5 lead card
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def round5_follow_logic(self,suit_played):
        ...
        ############################################################
        #round*5_follow_logic()'s point_sofar() function end ########


        # round*5_follow_logic() main body###################################
    ...
    #                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
            ...
            # debugging is going on - last round repeat
                ...
                # loading the previously played round*5 card
                f=open(self.cards_in_round_filenames[self.round_no],"rb")
                self.round_cards[self.round_no] = pickle.load(f)
...
#                                 input("\nHave to play trump card itself: 'Enter'")
                                ...
                                # the below lines have been commented out, since gui_round*5_card_played 
                                # needs to be called only once and only at the end of this 
                                # follow_logic() method
                                #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                                # gui window for round1 (follow card)
                                # self.gui_handle.gui_round*5_card_played(self.turn_index,self.card_played)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                self.player_input=self.gui_handle.gui_card_entry[self.round_no]()
                self.player_input=self.player_input.lower().strip()
#                 print("player inp in round*5 is :",self.player_input)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

                self.card_played=self.inp_parse_check_modified(self.player_input,self.x)

                # updating highest point sofar - need to check if this has to be used from round*5
...
#??????????????????????????????????????????????????????????????????????????????????
        ...
        # adding card to round*5_cards for saving in file
...
#         time.sleep(0.5)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round*5 (follow card)
    ...
    ####################################################################
    #round*5_follow_logic() method end #################################
    ...
    # round*5_play() method  #######################################
        ...
        while(len(self.obj_played_card_lst)<4):
            ...
            # calling the round*5 lead_logic for the first turn
            ...
            self.round5_follow_logic(self.round_lead_suit_index[self.round_no])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        print("\nRound-5 - starting from {}, counter_clockwise: ".format\
