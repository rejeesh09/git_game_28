...
class Round_7(Round_6):
    def __init__(self,hold,custom_deal,tkntr_rt):
        ...
        # kind of creating an object instance of Round_6() class with the above __init__()
        super().round6_play()
    ...
    def round7_lead_logic(self):
        # logic for opening turn of round
        
        self.turn_index=self.round7_lead_index
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round7 gui window for taking lead card input(i.e. if self.player leading the round)
            self.player_input=self.gui_handle.gui_round7_card_entry()
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            #15.######### var15
            self.round7_lead_card=self.inp_parse_check(self.player_input)
        else:
...
#         time.sleep(0.5)
        sys.stdout.write(self.round7_lead_card.show())
        ###############################################################  
        
        
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round7 lead card
        self.gui_handle.gui_card_played7(self.turn_index,self.round7_lead_card)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

        # adding inp to lst and dictionaries
        self.obj_played_card_lst.append(self.round7_lead_card)
        self.obj_dictn_of_highest_card_and_turn['suit']=[self.turn_index,self.round7_lead_card]

        self.obj_dictn_of_played_card_and_player[self.players_lst[self.turn_index]]\
        .append(self.round7_lead_card)

        self.obj_dictn_of_played_card_and_suit[self.round7_lead_card.suit()].append(self.round7_lead_card)
        
        # updating an additional dictionary from 18/09/2022 - not used
        self.obj_dictn_of_player_index_and_hand[self.turn_index].remove(self.round7_lead_card)
        
        #16.######### var16
        self.round7_lead_card_suit=self.round7_lead_card.suit()
        self.round7_lead_suit_index=self.suit_dictn[self.round7_lead_card_suit]
        #17.######### var17
        self.round7_highest_point_sofar=self.round7_lead_card.point()
        
        # removing card played from lst and dictn
        self.obj_deal_lst_copy[self.turn_index].remove(self.round7_lead_card)
        self.obj_dictn_of_cards_grouped[self.turn_index]\
                                        [self.round7_lead_suit_index].remove(self.round7_lead_card)    
    ###################################################################
    #round7_lead_logic() method end ###################################
    
    
    ###################################################################    
    def round7_follow_logic(self,suit_played):
        ...
        ############################################################
        #round7_follow_logic()'s point_sofar() function end ########


# round7_follow_logic() main body###################################
        ...
        if self.turn_index:
            ...
            if len(self.obj_dictn_of_cards_grouped[self.turn_index][self.x]) != 0:
                ...
                if abs(self.round7_lead_index - self.turn_index) == 2:
                    ...
                    if self.obj_dictn_of_cards_grouped[self.turn_index][self.x][-1].point()>\
                        self.round7_highest_point_sofar:
                        # updating highest point
                        self.round7_highest_point_sofar=self.card_played.point()
...
#                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
        ...
        # i.e self.turn_index==0
            ...
            played=False
#             time.sleep(0.5)
            if not len(self.obj_dictn_of_cards_grouped[0][self.round7_lead_suit_index]):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                    ...
                    self.check_val=self.gui_handle.gui_round7_trump_call_instance(self.turn_index)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                        ...
                        # the gui disp method of Widget() class in widget_manager module is called by its object
                        # gui_handle which was created earlier in __init__() of Deck()
                        self.gui_handle.gui_round7_trump_reveal(self.turn_index,\
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                # check conditions
                self.round7_highest_point_sofar=max(self.card_played.point(),self.round7_highest_point_sofar)
#??????????????????????????????????????????????????????????????????????????????????
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        self.gui_handle.gui_card_played7(self.turn_index,self.card_played)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    
    ###################################################################
    #round7_follow_logic() method end #################################
    
    
    def round7_play(self):
        # round7_play() method  #######################################
                
        while(len(self.obj_played_card_lst)<4):

            # calling the round7 lead_logic for the first turn
            if len(self.obj_played_card_lst)==0:                
                self.round7_lead_logic()
            ...
            self.round7_follow_logic(self.round7_lead_suit_index)


        # determining round8_lead_index
        ...
        self.round8_lead_index=key
        self.round8_lead_player=self.players_lst[key]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # the gui disp method of Widget() class in widget_manager module is called by its object
        # gui_handle which was created earlier in __init__() of Deck()
        self.gui_handle.gui_round7_summary(self.point_oppo_team,self.point_player_team,\
                                          self.round7_lead_player)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        
        time.sleep(0.5)
        print('\n')
        print(20*' '+'Mate:'+'{}'.format(self.obj_dictn_of_played_card_and_player['Mate'][6].show()))
        print('Oppo_left:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                       ['Oppo_left'][6].show()),end=' ')
        print(21*' '+'Oppo_right:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                               ['Oppo_right'][6].show()))
        print(20*' '+'{}:'.format(self.players_lst[0]),end=' ')
        print(self.obj_dictn_of_played_card_and_player[self.players_lst[0]][6].show())
        print("\nRound7 - starting from {}, counter_clockwise: ".format\
              (self.players_lst[self.round7_lead_index]),end=' ')        
        for i in self.obj_played_card_lst:
            print(i.show(),end=' ')
        print('')
        print('\nround8_lead_index: ',self.round8_lead_index)
