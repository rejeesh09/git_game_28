...
# Round_*2() class is where the second round of the game takes place(i.e. round*2_play() method in Round_*2())
# so the round*2_play() method has to be called from Round_*3 and round*3_play() from Round_*4 and so on
########################################################################
import time
import sys
...
class Round_3(Round_2):
    def __init__(self,hold,custom_deal,tkntr_rt):
        ...
        super().round2_play()
        # similarly round*3_play(to be defined){as well as super().__init__()} 
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round*3 gui window for taking lead card input(i.e. if self.player leading the round)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # point card or the minimum point card or a completely random card is played
            ...
            # debugging is going on
                ...
                # loading the previously played round*3 lead card
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round*3 lead card
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def round3_follow_logic(self,suit_played):
        ...
        ############################################################
        #round*3_follow_logic()'s point_sofar() function end ########


        # round*3_follow_logic() main body###################################
    ...
    #                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
            ...
            # debugging is going on - last round repeat
                ...
                # loading the previously played round*3 card
                f=open(self.cards_in_round_filenames[self.round_no],"rb")
                self.round_cards[self.round_no] = pickle.load(f)
...
#                                 input("\nHave to play trump card itself: 'Enter'")
                                ...
                                # the below lines have been commented out, since gui_round*3_card_played 
                                # needs to be called only once and only at the end of this 
                                # follow_logic() method
                                #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                                # gui window for round1 (follow card)
                                # self.gui_handle.gui_round*3_card_played(self.turn_index,self.card_played)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                # updating highest point sofar - need to check if this has to be used from round*3
...
#??????????????????????????????????????????????????????????????????????????????????
        ...
        # adding card to round*3_cards for saving in file
...
#         time.sleep(0.5)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round*3 (follow card)
    ...
    ####################################################################
    #round*3_follow_logic() method end #################################
    ...
    # round*3_play() method  #######################################
        ...
        while(len(self.obj_played_card_lst)<4):
            ...
            # calling the round*3 lead_logic for the first turn
            ...
            self.round3_follow_logic(self.round_lead_suit_index[self.round_no])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        print("\nRound-3 - starting from {}, counter_clockwise: ".format\
              (self.players_lst[self.round_lead_index[self.round_no]]),end=' ')        
        for i in self.obj_played_card_lst:
            print(i.show(),end=' ')
        print('')
        print('\nround-4_lead_index: ',self.round_lead_index[next_round])
