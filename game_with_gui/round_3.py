...
class Round_3(Round_2):
    ...
    def round3_lead_logic(self):
        # logic for opening turn of round
        
        self.turn_index=self.round3_lead_index
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round3 gui window for taking lead card input(i.e. if self.player leading the round)
            self.player_input=self.gui_handle.gui_round3_card_entry()
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            #15.######### var15
            self.round3_lead_card=self.inp_parse_check(self.player_input)
        else:
...
#         time.sleep(0.5)
        sys.stdout.write(self.round3_lead_card.show())
        ###############################################################  
        
        
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round3 lead card
        self.gui_handle.gui_card_played3(self.turn_index,self.round3_lead_card)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

        # adding inp to lst and dictionaries
        self.obj_played_card_lst.append(self.round3_lead_card)
        self.obj_dictn_of_highest_card_and_turn['suit']=[self.turn_index,self.round3_lead_card]

        self.obj_dictn_of_played_card_and_player[self.players_lst[self.turn_index]]\
        .append(self.round3_lead_card)

        self.obj_dictn_of_played_card_and_suit[self.round3_lead_card.suit()].append(self.round3_lead_card)
        
        # updating an additional dictionary from 18/09/2022
        self.obj_dictn_of_player_index_and_hand[self.turn_index].remove(self.round3_lead_card)
        
        #16.######### var16
        self.round3_lead_card_suit=self.round3_lead_card.suit()
        self.round3_lead_suit_index=self.suit_dictn[self.round3_lead_card_suit]
        #17.######### var17
        self.round3_highest_point_sofar=self.round3_lead_card.point()
        
        # removing card played from lst and dictn
        self.obj_deal_lst_copy[self.turn_index].remove(self.round3_lead_card)
        self.obj_dictn_of_cards_grouped[self.turn_index]\
                                        [self.round3_lead_suit_index].remove(self.round3_lead_card)    
    ###################################################################
    #round3_lead_logic() method end ###################################

    ###################################################################    
    def round3_follow_logic(self,suit_played):
...
# round3_follow_logic() main body###################################
        ...
        if self.turn_index:
            ...
            if len(self.obj_dictn_of_cards_grouped[self.turn_index][self.x]) != 0:
                ...
                if abs(self.round3_lead_index - self.turn_index) == 2:
...
#                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
        ...
        # i.e self.turn_index==0
            ...
            played=False
#             time.sleep(0.5)
            if not len(self.obj_dictn_of_cards_grouped[0][self.round3_lead_suit_index]):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                    ...
                    self.check_val=self.gui_handle.gui_round3_trump_call_instance(self.turn_index)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                        ...
                        # the gui disp method of Widget() class in widget_manager module is called by its object
                        # gui_handle which was created earlier in __init__() of Deck()
                        self.gui_handle.gui_round3_trump_reveal(self.turn_index,self.trump_card)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                # check conditions
                self.round3_highest_point_sofar=max(self.card_played.point(),self.round3_highest_point_sofar)
#??????????????????????????????????????????????????????????????????????????????????
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round1 (follow card)
        self.gui_handle.gui_card_played3(self.turn_index,self.card_played)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    
    ###################################################################
    #round3_follow_logic() method end #################################

    def round3_play(self):
        # round3_play() method  #######################################
                
        while(len(self.obj_played_card_lst)<4):

            # calling the round3 lead_logic for the first turn
            if len(self.obj_played_card_lst)==0:                
                self.round3_lead_logic()
            ...
            self.round3_follow_logic(self.round3_lead_suit_index)


        # determining round4_lead_index
        ...
        self.round4_lead_index=key
        self.round4_lead_player=self.players_lst[key]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # the gui disp method of Widget() class in widget_manager module is called by its object
        # gui_handle which was created earlier in __init__() of Deck()
        self.gui_handle.gui_round3_summary(self.point_oppo_team,self.point_player_team,\
                                          self.round4_lead_player)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        
        time.sleep(0.5)
        print('\n')
        print(20*' '+'Mate:'+'{}'.format(self.obj_dictn_of_played_card_and_player['Mate'][2].show()))
        print('Oppo_left:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                       ['Oppo_left'][2].show()),end=' ')
        print(21*' '+'Oppo_right:'+'{}'.format(self.obj_dictn_of_played_card_and_player\
                                               ['Oppo_right'][2].show()))
        print(20*' '+'{}:'.format(self.players_lst[0]),end=' ')
        print(self.obj_dictn_of_played_card_and_player[self.players_lst[0]][2].show())
        print("\nRound3 - starting from {}, counter_clockwise: ".format\
              (self.players_lst[self.round3_lead_index]),end=' ')        
        for i in self.obj_played_card_lst:
            print(i.show(),end=' ')
        print('')
        print('\nround4_lead_index: ',self.round4_lead_index)
