...
class Round_4(Round_3):
    ...
    def round4_lead_logic(self):
        ...
        self.turn_index=self.round4_lead_index
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            # round4 gui window for taking lead card input(i.e. if self.player leading the round)
            self.player_input=self.gui_handle.gui_round4_card_entry()
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            #15.######### var15
            self.round4_lead_card=self.inp_parse_check(self.player_input)
        else:
...
#         time.sleep(0.5)
        sys.stdout.write(self.round4_lead_card.show())
        ###############################################################  
        
        
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round4 lead card
        self.gui_handle.gui_card_played4(self.turn_index,self.round4_lead_card)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

        # adding inp to lst and dictionaries
        self.obj_played_card_lst.append(self.round4_lead_card)
        self.obj_dictn_of_highest_card_and_turn['suit']=[self.turn_index,self.round4_lead_card]

        self.obj_dictn_of_played_card_and_player[self.players_lst[self.turn_index]]\
        .append(self.round4_lead_card)

        self.obj_dictn_of_played_card_and_suit[self.round4_lead_card.suit()].append(self.round4_lead_card)
        
        # updating an additional dictionary from 18/09/2022 - not used
        self.obj_dictn_of_player_index_and_hand[self.turn_index].remove(self.round4_lead_card)
        
        #16.######### var16
        self.round4_lead_card_suit=self.round4_lead_card.suit()
        self.round4_lead_suit_index=self.suit_dictn[self.round4_lead_card_suit]
        #17.######### var17
        self.round4_highest_point_sofar=self.round4_lead_card.point()
        
        # removing card played from lst and dictn
        self.obj_deal_lst_copy[self.turn_index].remove(self.round4_lead_card)
        self.obj_dictn_of_cards_grouped[self.turn_index]\
                                        [self.round4_lead_suit_index].remove(self.round4_lead_card)    
    ###################################################################
    #round4_lead_logic() method end ###################################
...
# round4_follow_logic() main body###################################
        ...
        if self.turn_index:
            ...
            if len(self.obj_dictn_of_cards_grouped[self.turn_index][self.x]) != 0:
                ...
                if abs(self.round4_lead_index - self.turn_index) == 2:
...
#                 self.card_played=self.obj_dictn_of_players_and_hand[self.players_lst[self.turn_index]][-1]
        ...
        # i.e self.turn_index==0
            ...
            played=False
#             time.sleep(0.5)
            if not len(self.obj_dictn_of_cards_grouped[0][self.round4_lead_suit_index]):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                    ...
                    self.check_val=self.gui_handle.gui_round4_trump_call_instance(self.turn_index)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                        ...
                        # the gui disp method of Widget() class in widget_manager module is called by its object
                        # gui_handle which was created earlier in __init__() of Deck()
                        self.gui_handle.gui_round4_trump_reveal(self.turn_index,self.trump_card)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                # check conditions
                self.round4_highest_point_sofar=max(self.card_played.point(),self.round4_highest_point_sofar)
#??????????????????????????????????????????????????????????????????????????????????
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # gui window for round4 (follow card)
        self.gui_handle.gui_card_played4(self.turn_index,self.card_played)
        # the object gui_handle of the Widgets() class is created in Deck() class in 
        # deck.py module
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    
    ###################################################################
    #round4_follow_logic() method end #################################
    ...
    def round4_play(self):
        ...
        while(len(self.obj_played_card_lst)<4):
            ...
            self.round4_follow_logic(self.round4_lead_suit_index)
        ...
        self.round5_lead_index=key
        self.round5_lead_player=self.players_lst[key]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # the gui disp method of Widget() class in widget_manager module is called by its object
        # gui_handle which was created earlier in __init__() of Deck()
        self.gui_handle.gui_round4_summary(self.point_oppo_team,self.point_player_team,\
                                          self.round5_lead_player)
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        print("\nRound4 - starting from {}, counter_clockwise: ".format\
              (self.players_lst[self.round4_lead_index]),end=' ')        
        for i in self.obj_played_card_lst:
            print(i.show(),end=' ')
        print('')
        print('\nround5_lead_index: ',self.round5_lead_index)
