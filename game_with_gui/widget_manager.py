...
#             self.but_game_nxt.configure(command=lambda:some_var16.set(1))
    ...
    def gui_round7_card_played(self,turn_index,card_played):
        ...
        some_var14g=tk.IntVar() 

        self.but_r7_played_card_lst=[]
        ...
        for i in self.button_lst_4_crd[turn_index]:
            if i['text']==card_played.form():
                ...
                #to remove the widget displaying round7 lead
                self.lb_r7_lead.grid_forget()
                
                # a new button for the played card is formed and attached according to the 
                # turn_index and appended to the list which holds these buttons
                self.but_r7_played_card_lst.append(tk.Button(self.fr_game_center,text=card_played.form(),\
                                          fg=card_played.colour(),font=('GNU Unifont',15)))
                # attaching the buttons based on the turn index. Each button is given a name as detaching 
                # them by accessing through list index doesnot seem to work in gui_round5_summary() method
                if turn_index==0:
                    self.but_r7_player_card=self.but_r7_played_card_lst[-1]
                    self.but_r7_player_card.grid(column=1,row=2,pady=20)
                elif turn_index==1:
                    self.but_r7_right_card=self.but_r7_played_card_lst[-1]
                    self.but_r7_right_card.grid(column=2,row=1,padx=20)
                elif turn_index==2:
                    self.but_r7_mate_card=self.but_r7_played_card_lst[-1]
                    self.but_r7_mate_card.grid(column=1,row=0,pady=20)
                elif turn_index==3:
                    self.but_r7_left_card=self.but_r7_played_card_lst[-1]
                    self.but_r7_left_card.grid(column=0,row=1,padx=20)
                else:
                    print('\nSomething wrong abt turn_index')
        
                    
        self.but_game_nxt.configure(text='>>',command=lambda:some_var14g.set(1))
        self.but_game_nxt.grid(column=1,row=1)
        self.but_game_nxt.focus()
        
        self.fr_game_center.wait_variable(some_var14g)
        
    ############## gui_card_played7() end ############################################
    
    def gui_round7_card_entry(self):
        """
        This method is used whenever the player(not comp) has to input a card, 
        either while leading the round or following in the round
        The method brings up the widgets specific to taking input.
        The method is called from round7_lead_logic() and round7_follow_logic() in Round_7()
        """
        
        some_var16g=tk.IntVar()
        round7_card=tk.StringVar()
        
        def save_round7_card():
            global round7_entry
            round7_entry=round7_card.get()
        ...
        # this is nw in a frame inside the cell at (2,1) of the fr_game_center
        self.lab_round7_card=tk.Label(self.fr_game_center_bot,text='Enter card',font=('GNU Unifont',15))
        self.ent_round7_card=tk.Entry(self.fr_game_center_bot,textvariable=round7_card,font=('GNU Unifont',15))
        self.lab_round7_card.grid(row=0,column=0,sticky='')
        self.ent_round7_card.grid(row=0,column=1,sticky='')
        self.ent_round7_card.focus()

        self.but_round7_card=tk.Button(self.fr_game_center_bot,text='Enter', font=('GNU Unifont',15),\
                                 command=save_round7_card)        
        self.but_round7_card.grid(row=1,column=1,sticky='')
                        
        
        self.fr_game_center.wait_variable(some_var16g)
        
        return(round7_entry)
        
    ############## gui_round7_card_entry() end #######################################
    
    def gui_round7_trump_call_instance(self,turn_index):
        """
        This method deals with the situation of trump call by a player in round1
        This method is called from round7_follow_logic() in Round_7().
        """
        
        ret_var=tk.IntVar()
        
        # to remove widgets from round1, round2, round3, round4, round5, round6_card_entry
        # find some other way than adding each round labels here
        # this is done so because trump call instance happen only once and it can happen in any round
        # from 1 - 7, so each round currently need the below set of codes removing previous round widgets
        self.lab_round1_card.grid_forget()
        ...
        self.lab_round5_card.grid_forget()
        self.ent_round5_card.grid_forget()
        self.lab_round6_card.grid_forget()
        self.ent_round6_card.grid_forget()
    ...
    ############## gui_round7_trump_call_instance() end ##############################
    
    def gui_round7_trump_reveal(self,turn_index,trump_card,highest_bidder_index):
        """
        This method is called from round7_follow_logic() in Round_7()
        The method with the situ when trump is called (by comp hand) and revealed and the 
        trump_card/trump_card2 button is to be revealed i.e. replaced with the form() of trump_card
        ...
        def trump_reveal():
            ...
            if turn_index:
                # checking since lb_trump_call is not packed for turn_index==0
                lb_trump_call.grid_forget()
            self.but_game_nxt.configure(text='Trump revealed',command=lambda:some_var16g.set(1))
            self.but_game_nxt.focus()
    ...
    ############## gui_round7_trump_reveal() end #####################################
    
    def gui_round7_summary(self,point_oppo_team,point_player_team,round8_lead_player):
        """
        This method displays the summary at the end of round7, called from round7_play() in Round_7()
        """
    
        some_var17g=tk.IntVar()
        
        # button command
        def round8_lead():
            self.lb_point_oppo_team.grid_forget()
            self.lb_point_player_team.grid_forget()
            self.lb_r8_lead=tk.Label(self.fr_game_center,text='Round 8 starts with: '+round8_lead_player,\
                               font=('GNU Unifont',15))
            self.lb_r8_lead.grid(column=1,row=0,sticky='')
            self.but_game_nxt.configure(text='>>',command=lambda:some_var17g.set(1))
            self.but_game_nxt.focus()
        ...
        def points():
            # using grid_forget with list iteration does not seem to work, it detaches only the last 
            # button in list. Therefore each card in list is given separate name and detached (why?)
            self.but_r7_player_card.grid_forget()
            self.but_r7_right_card.grid_forget()
            self.but_r7_mate_card.grid_forget()
            self.but_r7_left_card.grid_forget()
            # labels for displaying each team points (frame: fr_game_center)
            self.lb_point_oppo_team=tk.Label(self.fr_game_center,text='Oppo_team: '+str(point_oppo_team),\
                                        font=('GNU Unifont',15))
            self.lb_point_player_team=tk.Label(self.fr_game_center,text='Your_team: '+str(point_player_team),\
            ...
            self.lb_point_player_team.grid(column=1,row=2,sticky='')
            # clickable next button (frame: fr_game_center)
            self.but_game_nxt.configure(text='>>',command=round8_lead)
            self.but_game_nxt.focus()
        ...
        self.fr_game_center.wait_variable(some_var17g)
        
    ############## gui_round7_summary() end ##########################################
